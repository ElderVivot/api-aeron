...
exports.LogNotaFiscalRepository = void 0;
const make_error_request_response_1 = require("../../../common/factories/make-error-request-response");
const connection_1 = require("../../../database/connection");
const common_1 = require("@nestjs/common");
let LogNotaFiscalRepository = class LogNotaFiscalRepository {
    constructor(connection) {
        this.connection = connection;
    }
    async index(dto, tenant) {
        const { idCompanie, modelNotaFiscal, situationNotaFiscal, dateStartDown, dateEndDown, typeLog, dateStartDownBetween, dateEndDownBetween, federalRegistration, codeCompanieAccountSystem, nameCompanie, typeSearch } = dto;
        try {
            let sql = `
    ...
    async indexToFront(dto, tenant) {
        const { _page, _limit, idCompanie, modelNotaFiscal, situationNotaFiscal, dateStartDown, dateEndDown, typeLog, dateStartDownBetween, dateEndDownBetween, codeCompanies, federalRegistration, codeCompanieAccountSystem, competence, nameCompanie } = dto;
        try {
            let sql = `
            ...
            return (0, make_error_request_response_1.MakeErrorRequestResponseV2)(`${tenant}.log_nota_fiscal`, 'indexToFront', __filename, error);
        }
    }
    async indexToFrontCountWithoutFilterPage(dto, tenant) {
        const { idCompanie, modelNotaFiscal, situationNotaFiscal, dateStartDown, dateEndDown, typeLog, dateStartDownBetween, dateEndDownBetween, codeCompanies, federalRegistration, codeCompanieAccountSystem, competence, nameCompanie } = dto;
        try {
            let sql = `
                SELECT COUNT(1)
                  FROM "${tenant}".log_nota_fiscal AS log_nota_fiscal
            ...
            return (0, make_error_request_response_1.MakeErrorRequestResponseV2)(`${tenant}.log_nota_fiscal`, 'indexToFrontCountWithoutFilterPage', __filename, error);
        }
    }
    async show(idLogNotaFiscal, tenant) {
        try {
            ...
            if (!result || result.length === 0)
                throw new common_1.NotFoundException(`LogNotaFiscal with ID ${idLogNotaFiscal} not found`);
            const client = result[0];
            return client;
        }
        catch (error) {
            return (0, make_error_request_response_1.MakeErrorRequestResponseV2)(`${tenant}.log_nota_fiscal`, 'show', __filename, error);
        }
    }
    ...
    async updateUrlPrintLog(idLogNotaFiscal, urlPrintLog, tenant) {
        try {
            const sql = `
                UPDATE "${tenant}".log_nota_fiscal
            ...
            return (0, make_error_request_response_1.MakeErrorRequestResponseV2)(`${tenant}.log_nota_fiscal`, 'updateUrlPrintLog', __filename, error);
        }
    }
    async getCompaniesThatDontProcessNotaFiscalYet(dto, tenant) {
        const { idCompanie, typeLog, federalRegistration, codeCompanieAccountSystem, typeFederalRegistration, idCompanieList } = dto;
        try {
            let sql = `
...
LogNotaFiscalRepository = __decorate([
    __param(0, (0, common_1.Inject)('CONNECTION')),
    __metadata("design:paramtypes", [connection_1.Connection])
], LogNotaFiscalRepository);
exports.LogNotaFiscalRepository = LogNotaFiscalRepository;
//# sourceMappingURL=log-nota-fiscal.repository.js.map