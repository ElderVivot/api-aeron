...
let LogNotaFiscalRepository = class LogNotaFiscalRepository {
    ...
    async indexToFront(dto, tenant) {
        const { _page, _limit, idCompanie, dateStartDown, dateEndDown, typeLog, dateStartDownBetween, dateEndDownBetween, codeCompanies, federalRegistration, codeCompanieAccountSystem, competence, nameCompanie } = dto;
        try {
            let sql = `
    ...
    async indexToFrontCountWithoutFilterPage(dto, tenant) {
        const { idCompanie, dateStartDown, dateEndDown, typeLog, dateStartDownBetween, dateEndDownBetween, codeCompanies, federalRegistration, codeCompanieAccountSystem, competence, nameCompanie } = dto;
        try {
            let sql = `
                SELECT COUNT(1)
                  FROM "${tenant}".log_nota_fiscal_go_ent AS log_nota_fiscal
    ...
    async show(idLogNotaFiscal, tenant) {
        try {
            ...
            return (0, make_error_request_response_1.MakeErrorRequestResponseV2)(`${tenant}.log_nota_fiscal_go_ent`, 'show', __filename, error);
        }
    }
    ...
    async getCompaniesThatDontProcessNotaFiscalYet(dto, tenant) {
        const { idCompanie, typeLog, federalRegistration, codeCompanieAccountSystem, idCompanieList } = dto;
        try {
            let sql = `
